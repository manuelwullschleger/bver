...
# Author: Yaqi Zhang
# Availability: https://github.com/zhangyaqi1989/Gcode-Reader
# Notice: Some parts of this code were modified by Marc Röthlisberger & Manuel Wullschleger
##################################
"""
...
class GcodeReader:
    ...
    def _read_lpbf_regular(self):
        ...
        self.powers = []
        temp = -float('inf')
        ngxyzfl = [temp, temp, temp, temp, temp, temp, temp, temp]
        d = dict(zip(['N', 'G', 'X', 'Y', 'Z', 'F', 'L', 'P'], range(8)))
        seg_count = 0
        for line in lines:
            old_ngxyzfl = ngxyzfl[:]
            tokens = line.split()
    ...
    def _read_fdm_regular(self):
        ...
        with open(self.filename, 'r') as infile:
            ...
            isTimelapse = False
            for line in lines:
                if (line.startswith('G') and isTimelapse == False):
                    idx = line.find(';')
                    if idx != -1:
                        line = line[:idx]
                    new_lines.append(line)
                if line.startswith(";TimeLapse Begin"):
                    isTimelapse = True
                if line.startswith(";TimeLapse End"):
                    isTimelapse = False
            lines = new_lines
        #pp.pprint(lines) # for debug
        self.segs = []
        ...
        d = dict(zip(['G', 'X', 'Y', 'Z', 'E', 'F', 'P'], range(7)))
        seg_count = 0
        for line in lines:
            old_gxyzef = gxyzef[:]
    ...
    def plot_layer(self, layer=1, ax=None, fig=None, linewidth=1):
